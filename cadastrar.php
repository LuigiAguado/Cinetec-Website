...
if($btnCadUsuario){
	...
	if(in_array('',$dados)){
		...
		$result_usuario = "SELECT id FROM login WHERE usuario='". $dados['usuario'] ."'";
		...
		$result_usuario = "SELECT id FROM login WHERE email='". $dados['email'] ."'";
		$resultado_usuario = mysqli_query($conn, $result_usuario);
		if(($resultado_usuario) AND ($resultado_usuario->num_rows != 0)){
			$erro = true;
		...
		<div class="container">
			<div class="form-signin" style="background: #fff;border-radius: 5px;padding: 7px; max-width: 500px;">
				...
				<form method="POST" action="">
					...
					<center><input type="submit" name="btnCadUsuario" value="Cadastrar" class="btn btn-success"></center>
					
					<div class="row text-center" style="margin-top: 20px;"> 
						Lembrou? <a class="criegra" href="login.php">Clique aqui</a> para logar
					</div>
				</form>
			</div>
		</div>
